...
def parse_file(file_name):
    ...
    while True:
        ...
        if len(arr) >= 5:
            '''[size, fct, goodput]'''
            results.append([int(arr[0]), float(arr[1]), float(arr[4])])
    f.close()
    return results
...
def print_result(results):
    ...
    print('%d flows/requests overall average completion time: %.5f us' % (len(results), average_fct_result(results)))
    print('%d flows/requests (0, 100KB) average completion time: %.5f us' % (len(small), average_fct_result(small)))
    print('%d flows/requests (0, 100KB) 99th percentile completion time: %.5f us' % (len(small), cdf_fct_result(small, 0.99)))
    print('%d flows/requests [100KB, 10MB) average completion time: %.5f us' % (len(medium), average_fct_result(medium)))
    print('%d flows/requests [10MB, ) average completion time: %.5f us' % (len(large), average_fct_result(large)))
    print('%d flows/requests overall average goodput: %.5f Mbps' % (len(results), average_goodput_result(results)))
