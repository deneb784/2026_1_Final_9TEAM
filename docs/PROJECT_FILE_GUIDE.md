...
| 경로 | 역할 |
|---|---|
| `pipeline/realtime/online_tg_flow_cache.py` | TrafficGenerator payload metadata를 이용해 실시간 flow를 식별한다. XDP packet event를 flow별로 모으고, feature packet count에 도달했는지 판단한다. |
...
| `pipeline/flow_cache.py` | 오프라인 dataset 생성용 FlowCache. `(src_index, flow_id, direction)` 기준으로 packet을 누적 |
| `pipeline/README.md` | pipeline 모듈 설명 |

주의:

- `pipeline/flow_cache.py`는 주로 오프라인 dataset 생성용이다.
- `pipeline/realtime/online_flow_cache.py`와 `online_tg_flow_cache.py`는 실시간 XDP 경로용이다.
