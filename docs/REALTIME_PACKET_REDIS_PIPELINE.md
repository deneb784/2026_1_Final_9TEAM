...
xdp/tg_xdp_capture.py
  -> flow_results 구독
  -> FlowCache entry 상태를 elephant/mice로 갱신
```
...
| `pipeline/realtime/online_request.py` | FlowEntry를 모델 request JSON으로 변환 |
...
| `pipeline/flow_cache.py` | flow 상태 및 classifier 결과 반영 |
...
`build_online_flow_request()`는 `FlowEntry`를 학습/추론 feature와 같은 형식으로 변환한다.

```text
1. FlowEntry.packets에서 tcp_len > 0인 packet만 사용
...
```json
{
  ...
  "logical_flow_id": "0:123:dst_to_src",
...
worker는 `payload` 안의 `x`, `feature_names`, `seq_len`, `request_key`를 사용한다.
...
```json
{
  ...
  "logical_flow_id": "3:1:dst_to_src",
  "run_id": "vl2_cuda_50",
  "inference_ms": 0.474,
  "score": 0.9993,
...
`tg_xdp_capture.py`는 `flow_results`를 백그라운드로 구독하고, `request_key`가 일치하는 FlowCache entry를 `elephant` 또는 `mice`로 갱신한다.
...
이 파일은 `tg_xdp_capture.py`가 Pub/Sub 결과를 받아 `FlowCache.apply_classification_result()`까지 성공한 뒤에만 append한다. 따라서 각 JSONL row는 다음 경로가 실제로 완료되었다는 증거다.
...
XDP feature ready
  ...
  -> FlowCache status/model_score/classification_result 반영
```
...
## 14. 온라인 결과 후처리 metric

온라인 worker 결과 JSONL에는 flow별 `score`, `predicted_label`, `inference_ms`가 저장된다. `recall`, `precision`, `f1`은 실험 종료 후 정답 source와 join해서 계산한다.

예측 threshold는 결과 JSONL의 `threshold`에서 자동으로 읽는다. `label`이 CDF 값이면 `--label-threshold` 이상을 elephant로 본다. 생략하면 예측 threshold와 같은 값을 사용한다. 고정 byte 기준이 필요하면 `--size-threshold-bytes`를 지정한다.

후처리 실행 명령은 `README.md`의 온라인 결과 후처리 metric 섹션을 따른다.
