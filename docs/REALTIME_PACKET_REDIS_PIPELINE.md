...
`src_index` 기반 `request_key`는 online 결과를 offline meta/dataset과 join해 recall/F1을 계산할 때 쓰는 보조 key다. 실시간 cache 반영은 `online_flow_key`로 수행한다.
...
worker는 `payload` 안의 `x`, `feature_names`, `seq_len`을 추론에 사용하고, 방향 정보는 `online_flow_key` 또는 `request_key`에서 읽는다. `request_key`는 metric 호환용 보조 key다.
...
`tg_xdp_capture.py`는 `flow_results`를 백그라운드로 구독하고, `online_flow_key`가 일치하는 실시간 FlowCache entry를 `elephant` 또는 `mice`로 갱신한다. `request_key`는 결과 JSONL에 남아 offline metric join에 사용된다.
