...
오프라인 데이터셋 생성은 `results/flows_*_meta.csv`와 `captured_packet/*.pcap`을 매칭해서 JSONL을 만듭니다. 실시간 XDP run은 기본적으로 pcap을 저장하지 않으므로, 데이터셋 생성용 run은 `--capture-mode tshark` 또는 검증용 `--capture-mode xdp-verify`를 사용합니다.
...
`--capture-mode xdp` 또는 `xdp-verify`에서 Redis를 켜면 `main.py`가 기본적으로 다음 파일을 남깁니다.
